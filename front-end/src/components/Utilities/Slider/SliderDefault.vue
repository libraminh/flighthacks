<template>
  ...
  >
    ...
    >
      <slot />
      <div
        v-if="$slots['special-item']"
        class="slider-item"
      >
        <slot name="special-item" />
      </div>
    </div>
    ...
    <div 
      v-if="hasControls || responsiveClass.controls" 
      :class="['tns-controls', responsiveClass.controls]"
    >
    ...
    <ul 
      v-if="hasNav" 
      :class="['tns-nav', responsiveClass.nav]"
    >
      <li 
      ...
      <li
        v-if="$slots['special-item']"
        :class="{'tns-nav-active': itemsCount === realIndex}"
        v-html="itemsCount + 1"
        @click="slider.goTo(itemsCount)"
      />
    </ul>
    ...
    <div :class="`${progressClass}`">
      ...
      >
        <span
          class="progress"
          :style="{width: `${parseInt(activeIndex) / itemsCount * 100}%`}"
        />
      </div>
    </div>
  </div>
